...
APP_STORE_CN_URL = (
    "https://apps.apple.com/cn/app/%E9%BB%84%E9%87%91%E5%BD%A2%E6%80%81%E9%80%9A-ai%E9%BB%84%E9%87%91%E5%A4%96%E6%B1%87%E6%9C%9F%E8%B4%A7%E8%A1%8C%E6%83%85"
    "-%E5%AE%9E%E6%97%B6%E8%A1%8C%E6%83%85%E5%8A%A9%E6%89%8B/id6749507728"
)
BRAND_HTML = (
    f'<a href="{APP_STORE_CN_URL}" target="_blank" rel="noopener noreferrer">'
    '<strong><span style="font-size:larger">黄金形态通</span></strong></a>'
)


def wrap_brand_in_post_html(text: str) -> str:
    if not text or "黄金形态通" not in text:
        return text
    return BRAND_HTML.join(text.split("黄金形态通"))
    ...
    for p in files[:limit]:
        try:
            ...
            if m:
                ...
                try:
                    titles.append(json.loads(raw_title))
                except json.JSONDecodeError:
                    if raw_title.startswith('"') and raw_title.endswith('"'):
                        titles.append(raw_title[1:-1])
                    else:
...
{preview}
    ...
    return f"""
        ...
        2. 正文用 3～4 个小节展开，使用 H2/H3；建议分别从盘面形态、技术指标、风险提示、交易/观察思路等角度展开，突出相关的指标数据、避免只复读价格数字。
...
def run_analysis():
    ...
    try:
        ...
        # JSON 字符串/数组也是合法 YAML 标量，可避免引号、冒号及 vol-high 等 Flow 解析歧义
        ...
        tags_yaml = json.dumps(all_tags, ensure_ascii=False)
        topics_yaml = "topics: []\n"
        if topics_zh:
            topics_yaml = "topics:\n" + "".join(
                f"  - {json.dumps(z, ensure_ascii=False)}\n" for z in topics_zh
            )
        ...
        post_body_markdown = (
            ...
            "- **App Store**：也可在 App Store 搜索「黄金形态通」。\n"
        ...
        post_md = (
            ...
            f"{topics_yaml}"
            "---\n\n"
            + wrap_brand_in_post_html(post_body_markdown)
        )
